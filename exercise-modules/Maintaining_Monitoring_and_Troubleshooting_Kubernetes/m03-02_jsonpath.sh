...
# We're working with the json output of our objects, in this cae Pods
...
# Display all Pod names, this will put the new line at the end of the set rather than on
#   each object output to screen.
# Additional tips on formatting code in the examples below including adding a new line
#   after each object
kubectl get pods -l app=hello-world -o jsonpath='{ .items[*].metadata.name }{"\n"}'
...
# Now that we're sorting that output, maybe we want a listing of all Pods sorted by a
#   field that's part of the object but not part of the default kubectl output. Like
#   creationTimestamp and we want to see what that value is
# We can use a custom column to output object field data, in this case the creation
#   timestamp
kubectl get pods -A -o jsonpath='{ .items[*].metadata.name }{"\n"}' \
...
# Range iterates over a list performing the formatting operations on each element in
#   the list
...
# We used --sortby when looking at Events earlier, let's use it for another something else
#   now...
...
# Adding in spaces or tabs in the output to make it a bit more readable
kubectl get pods -l app=hello-world -o jsonpath='{range .items[*]}{.metadata.name}{" "}{.spec.containers[*].image}{"\n"}{end}'
kubectl get pods -l app=hello-world -o jsonpath='{range .items[*]}{.metadata.name}{"\t"}{.spec.containers[*].image}{"\n"}{end}'
