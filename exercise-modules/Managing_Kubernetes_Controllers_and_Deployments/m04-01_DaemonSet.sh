...
# Let's create a DaemonSet with Pods on each node in our cluster...that's NOT the Control
# Plane Node
kubectl apply -f DaemonSet.yaml

# So we'll get three since we have 3 workers and 1 Control Plane Node in our cluster and
# the Control Plane Node is set to run only system pods
...
# Each Pod is created with our label, app=hello-world, controller-revision-hash and a
...
# Check on the status of our rollout, a touch slower than a deployment due to max
# Unavailable.
