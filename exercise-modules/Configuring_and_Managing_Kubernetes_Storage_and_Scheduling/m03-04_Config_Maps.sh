...
# This is how we can read in whole files at a time and present them to the file
#   system with the same name in one ConfigMap
...
kubectl exec -it $PODNAME -- /bin/sh
watch cat /etc/appconfig/appconfigqa # watch doesn't exist on nginx container
exit
...
#0 - Reading from a directory, each file's basename will be a key in the
#   ConfigMap...but you can define a key if needed
...
#1. Defining a custom key for a file. All configuration will be under that key in 
#   the filesystem.
...
kubectl delete configmap httpconfigprod1
