m03-02 Create User Certificate
...
# https://kubernetes.io/docs/concepts/cluster-administration/certifficates/#cfssl
# Create a private key
openssl genrsa -out demouser.key 2048

# Generate a CSR
#CN (Common Name) is your username O (Organization) is the Group
# If you get an error "Can't load /home/USERNAME/.rnd into RNG" - comment out RANDFILE
...
# Let's get the CSR to see its current state. The CSR will delete after an hour
...
# We're going to use this file to build a kubeconfig file to authenticate to the API
#   Server with
kubectl get certificatesigningrequests demouser \
...
# Read the certificate itself
# Key elements: Issuer is our CA, Validity one year, Subject CN=demouser
openssl x509 -in demouser.crt -text -noout | head -n 15

# Now that we have the certificate, we can use that to build a kubeconfig file with to
#   log into this cluster.
# We'lluse demouser.key and demouser.crt
