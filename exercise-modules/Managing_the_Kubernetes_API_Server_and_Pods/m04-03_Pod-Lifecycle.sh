...
kubectl exec -it hello-world-pod -- /bin/sh # need to figure out why this doesn't work
ps
exit

# we still have our kubectl get events running in the background, so we see if re-create the container automatically.
kubectl exec -it hello-world-pod -- /usr/bin/killall hello-app # this also doesn't work

# Our restart count increased by 1 after the container needed to be restarted.
kubectl get pods

# Look at Containers->State, Reason, Exit Code, Restart Count and Events
# This is because the container restart policy is Always be default
...
kubectl exec -it hello-world-never-pod -- /usr/bin/killall hello-app # still doesn't work
kubectl get pods

# Review cotainer state, reason, exit code, ready and conditions Ready, ContainerReady
